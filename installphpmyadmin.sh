...
echo "location /mysql {" >> /etc/nginx/sites-enabled/$subdomain;
echo "proxy_pass      http://localhost:81/mysql;" >> /etc/nginx/sites-enabled/$subdomain;
...
echo "return 301 https://$subdomain/mysql;" >> /etc/nginx/sites-enabled/$subdomain;
echo "}" >> /etc/nginx/sites-enabled/$subdomain;
echo "}" >> /etc/nginx/sites-enabled/$subdomain;
